...
#strategy 
#shift x to be 0 index, to match the doors, because doors are 0 index, keys should be as well
#You can only open 3 doors, if any 2 doors have no 0 key behind them
...
for _ in range(int(input())):
    x = int(input())
    x = x - 1
    abc = [a,b,c] = list(map(int, input().split()))
    if abc[x] != 0:
        x = abc[x] - 1
        if abc[x] != 0:
            print("YES")
